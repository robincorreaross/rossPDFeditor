...
class MainWindow(QMainWindow):
    ...
    def _rebuild_thumbnails(self):
        ...
        # Garante que o layout e container existem
        ...
        # 1. Ajustar número de widgets (reaproveita os que já existem)
        if target_count < current_widgets_count:
            # Remover excedentes do final
            for _ in range(current_widgets_count - target_count):
                thumb = self.thumbnails.pop()
                self.pages_layout.removeWidget(thumb)
                thumb.deleteLater()
        elif target_count > current_widgets_count:
            # Adicionar novos componentes
            for i in range(current_widgets_count, target_count):
                # Placeholder vazio, será preenchido no loop de atualização abaixo
                thumb = PageThumbnail(i, b"") 
    ...
    def _on_page_delete(self, index: int):
        ...
        # Otimização: Em vez de limpar o cache (O(N) re-render), apenas deslocamos os índices
        new_cache = {}
        for idx, data in self._thumbnail_cache.items():
            if idx < index:
                new_cache[idx] = data
            elif idx > index:
    ...
    def _on_page_duplicate(self, index: int):
        ...
        # Otimização: Deslocar cache para a direita para abrir espaço para a nova cópia
        new_cache = {}
        for idx, data in sorted(self._thumbnail_cache.items(), reverse=True):
            if idx > index:
                new_cache[idx + 1] = data
            else:
                new_cache[idx] = data
        
        # A nova página (index + 1) é idêntica à original
        if index in self._thumbnail_cache:
            new_cache[index + 1] = self._thumbnail_cache[index]
